...
function publicEnquiry(enquiry) {
  return {
    ...
    targetMarket: enquiry.targetMarket,
    buyerType: enquiry.buyerType,
    volume: enquiry.volume,
    packFormat: enquiry.packFormat,
    productRange: enquiry.productRange,
    packRequest: enquiry.packRequest,
    destinationPort: enquiry.destinationPort,
    timeline: enquiry.timeline,
    documentNeed: enquiry.documentNeed,
    labelRequirement: enquiry.labelRequirement,
    quoteBasis: enquiry.quoteBasis,
...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/wholesale" && req.method === "POST") {
    ...
    const enquiry = {
      ...
      targetMarket: text(payload.targetMarket, 120),
      buyerType: text(payload.buyerType, 120),
      volume: text(payload.volume, 80),
      packFormat: text(payload.packFormat, 120),
      productRange: text(payload.productRange, 120),
      packRequest: text(payload.packRequest, 180),
      destinationPort: text(payload.destinationPort, 160),
      timeline: text(payload.timeline, 120),
      documentNeed: text(payload.documentNeed, 160),
      labelRequirement: text(payload.labelRequirement, 160),
      quoteBasis: text(payload.quoteBasis, 80),
